...
def generate_key():
    ...
    """
    #  Generate a random 16-byte (128-bit) encryption key. (Hint: 1 line of code required)
    # See here for a hint: https://cryptography.io/en/latest/hazmat/primitives/symmetric-encryption.html
    #key = b'password12345678'  #  We need to use a good source of randomness for our key.
    key = os.urandom(16)
...
def encrypt(message):
    ...
    #  Generate and display a key using a function from above. (Hint: 1 line of code required)
    # key = b'password12345678' #  You must use the function above to generate a key.
    key = generate_key()

    print(f'Key: {key}')

    #  Encode the key as base64 using a function from above. (Hint: 1 line of code required)
    base64_encoded_key = base64_encode(key)
    print(f'\nBase64 encoded key: {base64_encoded_key}\n')

    # Convert the message to bytes using a function from above. (Hint: 1 line of code required)
    ...
    #  Create an 'encryptor'. (Hint: 1 line of code required)
    encryptor = cipher.encryptor()


    #  Update the encryptor with your message and finalize the encryptor. (Hint: 1 line  of code required)
    ciphertext = encryptor.update(message) + encryptor.finalize()

    #  Run 'finalize()' on the encryptor. (Hint: 1 line of code required)
    #ciphertext =   # Add ciphertext from above to encryptor.finalize() like in the documentation examples.

    # Print the ciphertext
    print(f'Ciphertext: {ciphertext}')

    # Display the base-64 encoded ciphertext. (Hint: 1 line of code required)
    base64_encoded_ciphertext = base64_encode(ciphertext) #'This is not base64 encoded yet.'
...
def decrypt(key, ciphertext):
    ...
    key_as_bytes = base64_decode(key)#'TODO - Decode the key from base64 to bytes using the function above'
    ...
    ciphertext_as_bytes = base64_decode(ciphertext)#'TODO - Decode the ciphertext from base64 to bytes using the function above'
    ...
    final_plaintext = decryptor.update(ciphertext_as_bytes) + decryptor.finalize()
